...
void quit_client(int s) { if (ui_pid > 0) kill(ui_pid, SIGKILL); exit(0); }

void run_display() {
    Msg m;
    while(1) {
        m.mtype = CHAN_SERVER; m.player_id = my_id; m.action = ACT_STATE;
        msgsnd(mid, &m, sizeof(Msg)-sizeof(long), 0);
        msgrcv(mid, &m, sizeof(Msg)-sizeof(long), (my_id == 1) ? CHAN_P1 : CHAN_P2, 0);

        print_s("\033[H\033[J");
        print_s("=== GRA STRATEGICZNA | ID: "); print_i(my_id); print_s(" ===\n");
        print_s("Punkty: "); print_i(m.score == -1 ? 0 : m.score); print_s("/"); print_i(WIN_THRESHOLD);
        print_s(" | Zloto: "); print_i(m.gold); print_s(" | "); print_s(m.active ? "TRWA\n" : "CZEKANIE\n");
        print_s("--------------------------------------------\n");
        for(int i=0; i<4; i++) {
        ...
        if (m.score >= WIN_THRESHOLD || m.score == -1) {
            int win_id = (m.score >= WIN_THRESHOLD) ? my_id : (my_id == 1 ? 2 : 1);
            print_s("\n\n   KONIEC GRY: WYGRAL GRACZ "); print_i(win_id); print_s("\n");
            sleep(1); kill(getppid(), SIGINT);
        }
        sleep(1);
    }
}
...
int main() {
    ...
    if (mid == -1) { print_s("Brak serwera.\n"); return 1; }

    Msg m;
    m.mtype = CHAN_SERVER; m.sender_pid = getpid(); m.action = ACT_LOGIN;
    msgsnd(mid, &m, sizeof(Msg)-sizeof(long), 0);
    ...
    while(1) {
        ...
        if (n <= 0) continue;
        buf[n-1] = '\0';
        int choice = str_to_i(buf);

        if (choice == 1) {
            kill(ui_pid, SIGSTOP);
            print_s("\n[BUDOWA] Typ: "); n = read(STDIN_FILENO, buf, 31); buf[n-1] = '\0';
            int t = str_to_i(buf);
            print_s("Ilosc: "); n = read(STDIN_FILENO, buf, 31); buf[n-1] = '\0';
            int q = str_to_i(buf);
