...
// Funkcja obsługi sygnału (np. Ctrl+C), która czyści procesy klienta
void quit_client(int s) { 
    if (ui_pid > 0) kill(ui_pid, SIGKILL); 
    exit(0); 
}

// PROCES POTOMNY: Odpowiada za cykliczne odświeżanie interfejsu
void run_display() {
    Msg m;
    while(1) {
        // Komunikacja z serwerem przez kolejki komunikatów
        m.mtype = CHAN_SERVER;
        ...
        print_s("\033[H\033[J"); // Czyszczenie ekranu (sekwencja ANSI)
        ...
        // --- POPRAWIONA LOGIKA KOŃCA GRY ---
        if (m.score >= WIN_THRESHOLD) {
            // Ty osiągnąłeś limit punktów
            print_s("\n\n============================================\n");
            print_s("                WYGRANA!                    \n");
            print_s("============================================\n");
            sleep(2);
            kill(getppid(), SIGINT); // Sygnał do procesu nadrzędnego o zamknięciu
            break;
        } 
        else if (m.score == -1) {
            // Serwer wysłał -1, bo przeciwnik wygrał
            print_s("\n\n============================================\n");
            print_s("               PRZEGRANA...                 \n");
            print_s("============================================\n");
            sleep(2);
        ...
        sleep(1); // Czekaj sekundę przed kolejnym odświeżeniem
    }
}
...
int main() {
    ...
    // Podłączenie do KOLEJKI KOMUNIKATÓW (IPC)
    mid = msgget(PROJECT_KEY, 0600);
    if (mid == -1) { 
        print_s("Blad: Serwer nie jest uruchomiony!\n"); 
        return 1; 
    }

    // Proces logowania do serwera
    Msg m;
    m.mtype = CHAN_SERVER;
    m.sender_pid = getpid();
    m.action = ACT_LOGIN;
    ...
    if (m.player_id == -1) { 
        print_s("Serwer pelny. Sprobuj pozniej.\n"); 
        return 0; 
    }
    my_player_id = m.player_id;

    // Utworzenie PROCESU potomnego dla wyświetlania (fork)
    if ((ui_pid = fork()) == 0) { 
        run_display(); 
        exit(0); 
    }

    // GŁÓWNA PĘTLA STEROWANIA (Proces macierzysty)
    char buffer[32];
    while(1) {
        ...
        if (choice == 1) {
            // Użycie SYGNAŁÓW do zatrzymania UI na czas wpisywania danych
            kill(ui_pid, SIGSTOP);
            print_s("\n[BUDOWA] Typ (0-3): "); 
            ...
            kill(ui_pid, SIGCONT); // Wznowienie procesu UI
        } 
        else if (choice == 2) {
            kill(ui_pid, SIGSTOP);
